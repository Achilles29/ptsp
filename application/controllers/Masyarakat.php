...
class Masyarakat extends CI_Controller
{
  public function __construct()
  {
    ...
    $this->load->model(['Antrian_model', 'Layanan_model', 'Jenislayanan_model', 'Operasional_model']);
  ...
  public function simpan_antrian()
  {
    ...
    $layanan = $this->db
      ->select('l.*, i.id as instansi_id, i.is_aktif as instansi_aktif, i.status_layanan, i.status_layanan_mode, i.jam_tutup_pendaftaran, i.jam_layanan_mulai, i.jam_layanan_selesai, i.jam_tutup_kantor')
    ...
    if (!$layanan || empty($layanan->id)) {
      $this->session->set_flashdata('error', 'Layanan tidak ditemukan.');
      redirect('masyarakat/daftar_antrian');
      return;
    }

    $validasi_online = $this->Operasional_model->validate_online_registration($layanan, $tanggal);
    if (!$validasi_online['allowed']) {
      $this->session->set_flashdata('error', $validasi_online['message']);
      redirect('masyarakat/daftar_antrian');
      return;
    }
