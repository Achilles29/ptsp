...
class Pendaftaran extends CI_Controller
{
    ...
    public function manual_v2_tab()
    {
        $data['title'] = 'Frontdesk Antrian - Tab';
        ...
        $this->load->view('pendaftaran/manual_v2_tab', $data);
    }
    ...
    public function generate_antrian_v2()
    {
        ...
        $instansi = $this->db
            ...
            ->join('instansi i', 'i.id = jl.instansi_id')
            ->where('jl.id', $layanan_id)
            ->get()
            ->row();

        if (!$instansi) {
            echo json_encode(['success' => false, 'message' => 'Instansi tidak ditemukan']);
            return;
        }

        $validasi = $this->Operasional_model->validate_offline_registration($instansi);
        if (!$validasi['allowed']) {
            echo json_encode(['success' => false, 'message' => $validasi['message']]);
            return;
        }
